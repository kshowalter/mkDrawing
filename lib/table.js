  ...
  mk: function(){
    ...
    for( r=1; r<=this.num_rows; r++ ){
      for( c=1; c<=this.num_cols; c++ ){
        ...
        if( cell.cell_text && cell.cell_text.constructor === Array ){
          var row_size_current = self.row_sizes[r];
          var row_size_calc = Math.ceil( self.row_sizes[r]*cell.cell_text.length );
          var row_size = row_size_calc > row_size_current ? row_size_calc : row_size_current;
          self.row_size(r, row_size);
        }
      }
    }
